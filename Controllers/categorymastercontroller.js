...
module.exports.categorymasterinsert = async (req, res) => {
    try {
        ...
        const newCategory = await categorymastermodel.create({ category_id, category_name, category_image, description });
...
module.exports.categoryuploadmultipleimage = async (req, res, next) => {
  try {
      let uploadedFileNames = [];
      const filePath = path.join(__dirname + '/categoryImageUpload/Images');
      ...
      const storage = multer.diskStorage({
          ...
          filename: (req, file, cb) => {
              ...
              const uploadedFileNameWithPath = '/categoryImageUpload/Images/' + newFilename;
              uploadedFileNames.push(uploadedFileNameWithPath);
              cb(null, newFilename);
          }
      });

      const upload = multer({ storage: storage }).array('category_image', 6); // 'images' is the field name in your form, and 5 is the maximum number of files allowed
